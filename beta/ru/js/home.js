...
const Home = {
    ...
    _makeCard(c) {
        ...
        card.innerHTML = `
      <div class="container-drag-handle" title="Перетащите для сортировки">
      ...
      <div class="container-card-body">
        ...
        <div class="container-card-sizes">
          <span>${fmtSize(c.totalSize || 0)} использовано</span>
          <span>${fmtSize(Math.max(0, CONTAINER_LIMIT - (c.totalSize || 0)))} свободно</span>
        </div>
      </div>
      ${hasSess ? `<div class="session-badge" title="Активная сессия — нажмите для возобновления">
      ...
      <div class="container-card-menu" data-id="${c.id}" title="Параметры">
...
function showContainerMenu(e, c) {
    ...
    items.push({
        label: 'Открыть', icon: Icons.unlock,
    ...
    if (hasSess) {
        items.push({ label: 'Завершить сессию', icon: Icons.lock, action: () => killSession(c) });
    }

    // Change Password — always visible, disabled when session active or container open
    const cpDisabled = hasSess || isOpen;
    items.push({
        label: 'Изменить пароль…', icon: Icons.key,
        _keyHint: !cpDisabled ? '<span class="auth-dot"></span>' : null,
        disabled: cpDisabled,
        _tooltip: cpDisabled ? 'Сначала завершите активную сессию' : null,
    ...
    items.push({
        label: 'Переименовать…', icon: Icons.rename, disabled: rnDisabled,
        _tooltip: rnDisabled ? 'Сначала завершите активную сессию' : null,
        action: rnDisabled ? null : () => openRenameContainerModal(c)
    });

    items.push({ sep: true });
    items.push({
        label: 'Экспорт контейнера', icon: Icons.download,
        _keyHint: reqExpPw ? '<span class="auth-dot"></span>' : null,
        action: () => exportContainerFile(c, reqExpPw)
    });
    items.push({ sep: true });
    items.push({ label: 'Удалить контейнер...', icon: Icons.trash, danger: true, action: () => confirmDeleteContainer(c) });
...
function killSession(c) {
    ...
    clearSession(c.id);
    toast(`Сессия контейнера"${c.name}" завершена`, 'info');
    Home.render();
}
...
async function doChangePassword() {
    ...
    if (!oldPw) { errEl.textContent = 'Введите текущий пароль'; return; }
    if (newPw.length < 4) { errEl.textContent = 'Новый пароль должен содержать не менее 4 символов'; return; }
    if (newPw !== newPw2) { errEl.textContent = 'Пароли не совпадают'; return; }
    if (oldPw === newPw) { errEl.textContent = 'Новый пароль должен отличаться от текущего'; return; }
    ...
    try {
        ...
        if (!ok) {
            ...
            if (cpFails.count > 3) {
                ...
                errEl.innerHTML = `${_ERR_SVG} Неверный текущий пароль`;
        ...
        Overlay.hide();
        showLoading('Формирование нового ключ…');
        ...
        // Re-encrypt VFS
        showLoading('Повторное шифрование VFS…');
        ...
        if (c.lazyWorkspace) {
            showLoading('Восстановление файлов из импорта\u2026');
        ...
        showLoading(`Повторное шифрование файлов\u2026 0/${files.length}`);
        let _reencDone = 0;
        const reencResults = await Promise.allSettled(files.map(async f => {
            ...
            if (_reencDone % 4 === 0 || _reencDone === files.length)
                showLoading(`Повторное шифрование файлов\u2026 ${_reencDone}/${files.length}`);
        ...
        hideLoading();
        toast(`Пароль для «${c.name}» успешно изменён`, 'success');
        Home.render();
    } catch (e) {
        if (_cpInModal) {
            okBtn.disabled = false;
...
async function doRenameContainer() {
    ...
    const nameRegex = /^[a-zA-Zа-яА-ЯёЁЗ0-9 _\-.,!()@#$%&+=]+$/;
    if (!nameRegex.test(name)) { errEl.textContent = 'Недопустимые символы в названии'; return; }
    ...
    if (existing.find(x => x.id !== c.id && x.name.toLowerCase() === name.toLowerCase())) {
        errEl.textContent = 'Контейнер с таким названием уже существует'; return;
    }

    c.name = name;
    await DB.saveContainer(c);
    Overlay.hide();
    toast(`Контейнер переименован в «${name}»`, 'success');
    Home.render();
}
...
function confirmDeleteContainer(c) {
    ...
    document.getElementById('dc-msg').textContent =
        `Контейнер «${c.name}» и ВСЕ его файлы будут безвозвратно удалены. Это нельзя отменить.`;
    ...
    const timer = setInterval(() => {
        ...
        if (remaining > 0) {
            ...
            clearInterval(timer);
            okBtn.disabled = false;
            okLabel.textContent = 'Удалить навсегда';
        }
    }, 1000);
...
const _HW_STATES = {
    idle: () => `${_KEY_ICON}<span>Использовать ключ доступа для соли</span>`,
    loading: () => `<div class="spinner" style="width:11px;height:11px;border-width:1.5px;flex-shrink:0"></div><span>Подключение…</span>`,
    ...
    fail: () => `${_KEY_ICON}<span>Использовать ключ доступа для соли</span>`,
    unavailable: () => `${_KEY_ICON}<span>Использовать ключ доступа для соли</span>`,
};
...
function openNewContainerModal() {
    ...
    if (hwBtn) {
        ...
        if (unavailReason) {
            ...
            _setHwBtn('idle');
            hwBtn.title = 'Добавить энтропию пасски в соль';
            ...
            if (typeof PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable === 'function') {
                PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable()
                    .then(ok => {
                        if (!ok) {
                            _setHwBtn('unavailable');
                            hwBtn.title = 'Платформенный аутентификатор недоступен на этом устройстве';
                        }
                    })
                    .catch(() => {
                        _setHwBtn('unavailable');
                        hwBtn.title = 'Проверка платформенного аутентификатора не удалась';
                    });
            }
        }
    }
...
async function createContainer() {
    ...
    if (!name) { toast('Введите название контейнера', 'error'); return; }
    // Allow only letters (Latin + Cyrillic), digits, spaces, and safe filename chars
    const nameRegex = /^[a-zA-Zа-яА-ЯёЁ0-9 _\-.,!()@#$%&+=]+$/;
    if (!nameRegex.test(name)) {
        toast('Унедопустимые символы в названии. Используйте буквы, цифры, пробелы и безопасные символы', 'error'); return;
    } if (!document.getElementById('nc-agree').checked) { toast('Пожалуйста, примите условия ответственности за данные', 'error'); return; } if (pw.length < 4) { toast('Пароль должен содержать не менее 4 символов', 'error'); return; }
    if (pw !== pw2) { toast('Пароли не совпадают', 'error'); return; }

    const existing = await DB.getContainers();
    if (existing.find(c => c.name.toLowerCase() === name.toLowerCase())) {
        toast('Контейнер с таким названием уже существует', 'error'); return;
    }

    // Check device storage before creating
    const spCheck = await checkStorageSpace(1024 * 1024); // minimal overhead
    if (!spCheck.ok) {
        toast(`Недостаточно места на устройстве (${fmtSize(spCheck.available)} доступно)`, 'error'); return;
    }

    showLoading('Создание контейнера и вывод ключа...');
    Overlay.hide();
    try {
        ...
        toast(`Контейнер «${name}» создан`, 'success');
        await Home.render();
...
async function doUnlock() {
    ...
    if (!pw) { document.getElementById('unlock-error').innerHTML = '<svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M8 1.5a6.5 6.5 0 100 13 6.5 6.5 0 000-13zM7.25 5h1.5v4h-1.5V5zm0 5h1.5v1.5h-1.5V10z" fill="currentColor"/></svg> Введите пароль'; return; }
    ...
    try {
        ...
        if (!ok) {
            ...
            if (fails.count > 3) {
                ...
                document.getElementById('unlock-error').innerHTML = '<svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M8 1.5a6.5 6.5 0 100 13 6.5 6.5 0 000-13zM7.25 5h1.5v4h-1.5V5zm0 5h1.5v1.5h-1.5V10z" fill="currentColor"/></svg> Неверный пароль';
        ...
        if (c.lazyWorkspace) {
            ...
            try {
                ...
                toast('Не удалось восстановить файлы из импорта', 'error');
            }
            hideLoading();
        }
        ...
        toast(`Контейнер «${c.name}» разблокирован`, 'success');
        ...
        if (remEl && remEl.checked) {
            ...
            try {
                ...
                toast('Сессию не удалось сохранить — при следующем входе потребуется ввести пароль вновь', 'warn');
            }
        } else {
...
async function deleteContainerConfirmed() {
    ...
    showLoading('Удаление контейнера...');
    Overlay.hide();
    try {
        ...
        hideLoading();
        await Home.render();
        toast(`Контейнер «${c.name}» удалён`, 'info');
    } catch (e) {
        hideLoading();
...
async function _resumeSession(c, rawKeyBytes) {
    ...
    try {
        ...
        Desktop.render();
        if (typeof _cleanOrphanedExportCache === 'function') _cleanOrphanedExportCache().catch(() => { });
        toast(`Сессия «${c.name}» восстановлена`, 'success');
    } catch (e) {
        hideLoading();
