...
function _startAttemptCooldown(errEl, btn, onClear) {
    let remaining = 3;
    const upd = s => { errEl.innerHTML = `${_WAIT_SVG} Слишком много попыток — подождите ${s}с`; errEl.style.color = 'var(--orange)'; };
    upd(remaining);
    btn.disabled = true;
...
/* ============================================================
   ...
   ============================================================ */
const FOLDER_COLORS = [
    { label: 'По умолчанию (синий)', color: '#0078d4' },
    { label: 'Бирюзовый', color: '#4ec9b0' },
    { label: 'Фиолетовый', color: '#9b59d0' },
    { label: 'Оранжевый', color: '#f18800' },
    { label: 'Красный', color: '#e84040' },
    { label: 'Зелёный', color: '#3cb371' },
    { label: 'Розовый', color: '#e879a0' },
    { label: 'Жёлтый', color: '#d4b030' },
    { label: 'Серый', color: '#7a7a7a' },
];
