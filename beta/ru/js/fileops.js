...
async function uploadFiles(files) {
    ...
    if (totalNew > remaining) {
        toast(`Недостаточно места в контейнере. Нужно ${fmtSize(totalNew)}, доступно ${fmtSize(remaining)}`, 'error');
        return;
    }
    ...
    if (!spCheck.ok) {
        toast(
            `Недостаточно места на устройстве. Нужно ≈${fmtSize(totalNew)}, свободно только ${fmtSize(spCheck.available)}.`,
            'error'
        );
        return;
    }

    showLoading(`Шифрование ${files.length} файл${files.length > 1 ? 'ов' : 'а'}...`);
    let ok = 0, _okIds = [];
    ...
    for (let i = 0; i < fileArr.length; i += BATCH) {
        ...
        for (let j = 0; j < results.length; j++) {
            if (results[j].status === 'fulfilled') {
                ...
                toast('Ошибка шифрования: ' + batch[j].name, 'error');
            }
        }
        if (recs.length) await DB.saveFiles(recs);
        showLoading(`Шифрование... ${Math.min(i + BATCH, fileArr.length)}/${fileArr.length}`);
    }
    await saveVFS();
    Desktop._patchIcons();
    hideLoading();
    if (ok > 0) {
        toast(`${ok} файл${ok > 1 ? 'ов' : ''} импортировано`, 'success');
...
async function _uploadFileEntry(fileEntry, targetFolderId) {
    ...
    if (VFS.hasChildNamed(targetFolderId, name)) {
        toast(`"${name}" уже существует — пропущено`, 'warn');
        return false;
    }
...
async function _uploadDirEntry(dirEntry, targetFolderId, depth) {
    if (depth > 32) { toast('Слишком глубокая вложенность папок — остановлено на 32 уровнях', 'warn'); return false; }
    const name = sanitizeFilename(dirEntry.name);
    if (VFS.hasChildNamed(targetFolderId, name)) {
        toast(`Папка "${name}" уже существует — пропущено`, 'warn');
        return false;
    }
...
async function uploadEntries(dataTransferItems, targetFolderId) {
    ...
    const label = [
        fileEntries.length && `${fileEntries.length} файл${fileEntries.length !== 1 ? 'ов' : ''}`,
        folderEntries.length && `${folderEntries.length} папк${folderEntries.length !== 1 ? 'и' : 'а'}`,
    ].filter(Boolean).join(' и ');

    showLoading(`Шифрование ${label}…`);
    let ok = 0;
    for (const entry of entries) {
        try {
            ...
            toast(`Ошибка импорта: ${entry.name}`, 'error');
        }
    }
    await saveVFS();
    Desktop._patchIcons();
    ...
    hideLoading();
    if (ok > 0) {
        toast(`${ok} элемент${ok !== 1 ? 'ов' : ''} импортировано`, 'success');
        {
    ...
    if (_uploadLimitHit) toast(`Контейнер полон (${fmtSize(CONTAINER_LIMIT)}) — некоторые файлы не импортированы`, 'error');
    if (_uploadDeviceFullHit) toast('Недостаточно места на устройстве — некоторые файлы не импортированы', 'error');
}
...
async function openFile(node) {
    if (!App.key || !App.container) return;
    showLoading('Дешифрование файла...');
    try {
        const rec = await DB.getFile(node.id);
        if (!rec) { toast('Данные файла не найдены', 'error'); hideLoading(); return; }
...
async function deleteSelected() {
    ...
    const msg = ids.length === 1
        ? `Удалить «${names[0]}»? Действие необратимо.`
        : `Удалить ${ids.length} элементов? Действие необратимо.`;
...
function newTextFile() {
    ...
    let name = 'Документ.txt';
    if (VFS.hasChildNamed(targetFolder, name)) {
        let i = 2;
        while (VFS.hasChildNamed(targetFolder, `Документ (${i}).txt`)) i++;
        name = `Документ (${i}).txt`;
    }
...
async function createTextFile() {
    ...
    if (!name || name === 'unnamed') { toast('Введите допустимое имя файла', 'error'); return; }
    ...
    toast(`Файл «${name}» создан`, 'success');
...
function newFolder() {
    ...
    if (VFS.hasChildNamed(targetFolder, name)) {
        let i = 2;
        while (VFS.hasChildNamed(targetFolder, `Новая папка (${i})`)) i++;
        name = `Новая папка (${i})`;
    }
...
async function createFolder() {
    ...
    if (!name || name === 'unnamed') { toast('Введите допустимое имя папки', 'error'); return; }
    // Capture context BEFORE Overlay.hide() clears it
    const targetFolder = App.folder,
        winCtx = App._winCtx;
    // Duplicate name check
...
function renameNode(node) {
    ...
    if (node.type === 'folder') {
        ...
        if (blocked) {
            toast(`«${node.name}» открыта в Проводнике — сначала закройте окно`, 'error');
            return;
        }
    }
    ...
    document.getElementById('rename-ok').onclick = async () => {
        ...
        if (!newName || newName === 'unnamed') { toast('Введите допустимое имя', 'error'); return; }
...
function copyItems() {
    ...
    toast(`Скопировано: ${App.clipboard.ids.length} элементов`, 'info');
...
function cutItems() {
    ...
    toast(`Вырезано: ${App.clipboard.ids.length} элементов`, 'info');
...
async function pasteItems() {
    ...
    for (const id of ids) {
        ...
        if (op === 'cut') {
            ...
            if (result === 'duplicate') { toast(`«${n.name}» уже существует в этой папке`, 'error'); continue; }
            if (result === 'cycle') { toast(`Нельзя вставить «${n.name}» в себя или вложенную папку`, 'error'); continue; }
            _pastedSn = n.name;
            _pastedIds.push(id);
        } else {
            let name = n.name;
...
async function openFileAsText(node) {
    ...
    try {
        ...
        if (e.message === 'timeout') {
            toast('Операция превысила время ожидания (5 секунд)', 'warn');
        } else {
            toast('Ошибка дешифрования: ' + e.message, 'error');
        }
    }
}
...
function showProps(node) {
    ...
    <table class="props-table">
      <tr><td>Название</td><td>${escHtml(node.name)}</td></tr>
      <tr><td>Путь</td><td>${escHtml(VFS.fullPath(node.id))}</td></tr>
      <tr><td>Тип</td><td>${escHtml(node.type === 'folder' ? 'Папка' : (node.mime || getMime(node.name)))}</td></tr>
      ${node.size != null ? `<tr><td>Размер</td><td>${fmtSize(node.size)}</td></tr>` : ''}
      ${folderSz !== null ? `<tr><td>Размер</td><td>${fmtSize(folderSz)}</td></tr>` : ''}
      <tr><td>Создан</td><td>${fmtDate(node.ctime)}</td></tr>
      <tr><td>Изменён</td><td>${fmtDate(node.mtime)}</td></tr>
      ${node.type === 'folder' ? `<tr><td>Элементов</td><td>${VFS.children(node.id).length}</td></tr>` : ''}
      <tr><td>Шифрование</td><td style="color:var(--accent)">AES-256-GCM ✓</td></tr>
    </table>
  `;
...
async function saveEditor() {
    ...
    if (_delta > 0 && VFS.totalSize() + _delta > CONTAINER_LIMIT) {
        toast(`Невозможно сохранить: достигнут лимит контейнера (${fmtSize(CONTAINER_LIMIT)})`, 'error');
        return;
    }
    const needed = buf.byteLength * 1.1;
    const spCheck = await checkStorageSpace(needed);
    if (!spCheck.ok) {
        toast(`Невозможно сохранить: недостаточно места на устройстве (свободно ${fmtSize(spCheck.available)})`, 'error');
        return;
    }
    ...
    try {
        ...
        _scheduleExportCacheRefresh();
        saved = true;
    } catch (e) { toast('Ошибка сохранения: ' + e.message, 'error'); console.error(e); }
    hideLoading();
    return saved;
}
...
async function exportAsZip(nodeIds, zipName) {
    ...
    try {
        ...
        if (!entries.length) { toast('Нечего экспортировать', 'warn'); hideLoading(); return; }
        const zipParts = _buildZip(entries);
        downloadBuf(zipParts, zipName, 'application/zip');
        toast(`Экспортировано ${entries.length} файл${entries.length === 1 ? '' : entries.length < 5 ? 'а' : 'ов'} в ZIP`, 'success');
    ...
    } catch (e) { toast('Ошибка ZIP-экспорта: ' + e.message, 'error'); console.error(e); }
    hideLoading();
}
...
function _askExportPassword(c) {
    return new Promise(resolve => {
        ...
        const doExport = async () => {
            ...
            if (!pw) { errEl.innerHTML = _ERR_SVG + ' Введите пароль'; return; }
            errEl.innerHTML = '';
            btnOk.disabled = true;
            try {
                ...
                if (!ok) {
                    ...
                    if (fails.count > 3) {
                        ...
                        errEl.innerHTML = _ERR_SVG + ' Неверный пароль';
                        btnOk.disabled = false;
                    }
                    return;
                }
...
async function _updateExportCache(generateNow = false, silent = false) {
    ...
    if (!silent) showLoading('Создание кэша экспорта…');
    try {
        _setMsg('Кэш экспорта: чтение списка файлов…');
        const meta = await DB.getFileMetaByCid(App.container.id);
        let entries = meta;
        if (meta.some(m => m.sz < 0)) {
            _setMsg('Кэш экспорта: загрузка данных…');
        ...
        _setMsg(`Кэш экспорта: формирование манифеста (${entries.length} файл${entries.length === 1 ? '' : entries.length < 5 ? 'а' : 'ов'})…`);
...
async function exportContainerFile(c, requirePassword = true) {
    showLoading('Экспорт контейнера…');
    try {
    ...
    } catch (e) { toast('Ошибка экспорта: ' + e.message, 'error'); console.error(e); }
    hideLoading();
}
...
async function importContainerFile(file) {
    ...
    if (file.size > MAX_IMPORT_SIZE) {
        toast(`Файл импорта слишком большой (макс. ${fmtSize(MAX_IMPORT_SIZE)})`, 'error');
        return;
    }
    showLoading('Импорт контейнера…');
    try {
        ...
        if (isZip) {
            ...
            if (!nameRaw || !saltB64 || !verIvB64 || !verBlob)
                throw new Error('Неверный container.xml: отсутствуют обязательные поля');
            ...
            if (entries['meta/2'] && entries['meta/3']) {
                ...
                hideLoading();
                toast(`Контейнер «${name}» импортирован`, 'success');
                await Home.render();
                _highlightCard(newCid);
                return;
            }

            throw new Error('Неподдерживаемый формат контейнера. Поддерживаются только экспорты SafeNova v3 (.safenova).');

        } else {
            throw new Error('Неподдерживаемый формат файла. Используйте файл экспорта .safenova.');
        }
    } catch (e) {
        hideLoading();
