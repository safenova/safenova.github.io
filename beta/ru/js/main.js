...
(() => {
    ...
    setInterval(() => {
        if (Date.now() - _lastAlive > 3000 && typeof App !== 'undefined' && App.container && !_dmsLocking) {
            ...
            try { window.__snvEmergencyLock?.("\u0421траж безопасности перестал отвечать — контейнер заблокирован для безопасности."); } catch { }
            toast('Страж безопасности перестал отвечать — контейнер заблокирован для безопасности.', 'warn');
    ...
    if (App.container && e.key === 'snv-open-' + App.container.id) {
        try {
            ...
            if (d && d.tab !== _TAB_ID && d.kick) {
                App.lockContainer();
                toast('Контейнер открыт в другой вкладке — сессия завершена.', 'warn');
            }
